...
async def search_youtube(query: str, limit=20) -> list:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            url = f"https://invidious.privacyredirect.com/api/v1/search?q={query.replace(' ', '+')}&type=video"
...
async def cb_download(callback: types.CallbackQuery):
    ...
    try:
        ...
        if video_id:
            ...
            if dl_url:
                await bot.send_audio(uid, audio=dl_url, title=title[:64], performer=artist, caption=f"🎵 {title} - {artist}")
                await status.delete()
                return
        
        if preview:
            await bot.send_audio(uid, audio=preview, title=title[:64], performer=artist, caption=f"🎵 {title} - {artist}
⚠️ 30 soniyalik preview")
            await status.delete()
            return
        
        url = song.get("url", "")
        await status.edit_text(f"🔗 <a href="{url}">Tinglash uchun bosing</a>")
    except Exception as e:
