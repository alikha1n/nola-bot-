...
def detect_media_link(text: str) -> str | None:
    """Instagram/TikTok havolasini aniqlaydi (YouTube YO'Q - nagruzka uchun)."""
    patterns = [
...
async def handle_text(message: types.Message):
    ...
    if media_url:
        status = await message.answer("⏳ Video yuklanmoqda... / Загрузка...")
        link_key = "lnk_" + hashlib.md5(media_url.encode()).hexdigest()
        try:
            # 1) VIDEO ni yuklab yuboramiz
            video_path = await get_media_video(media_url)
            if video_path:
                # link ni saqlaymiz (tugmalar uchun)
                media_link_cache[uid] = media_url
                kb = InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🎵 Musiqasini topish", callback_data="getsong")],
                    [InlineKeyboardButton(text="🎧 Audiosini yuklash", callback_data="getaudio")],
                ])
                try:
                    await bot.send_video(
                        uid, video=FSInputFile(video_path),
                        caption="📹 Video tayyor!\n\nQuyidagidan tanlang:\n\n🤖 @nolamusicbot",
                        reply_markup=kb
                    )
                    await status.delete()
                except Exception as e:
                    logger.error(f"send video: {e}")
                    await status.edit_text(t(uid, "error"))
                finally:
                    try:
                ...
                await status.edit_text("❌ Video yuklab bo'lmadi (45MB dan katta yoki yopiq).")
        except Exception as e:
