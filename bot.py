...
REQUIRED_CHANNELS = []
WEBHOOK_HOST = "https://nola-bot.onrender.com"
...
TEXTS = {
    ...
    "en": {
        ...
        "sub_required": "📢 Subscribe to channel:",
        "sub_ok": "✅ Thank you!",
...
# ==================== DOWNLOAD ====================
async def download_full_song(query: str, uid: int) -> str | None:
    import yt_dlp
    import os
    output_path = f"/tmp/song_{uid}"
    ydl_opts = {
    ...
    try:
        ...
        if preview:
            await bot.send_audio(
                ...
                caption=f"🎵 {title} — {artist}\n\n⚠️ 30 soniyalik preview (Deezer)"
            )
            await status.delete()
        else:
            url = song.get("url", "")
            await status.edit_text(f"🔗 <a href=\"{url}\">Deezer da tinglash</a>")
    except Exception as e:
...
async def cb_channels(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(f"📢 <b>Kanallar:</b>\n{channels}", reply_markup=admin_keyboard())
    await callback.answer()
...
async def cb_bc_channel(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("📣 Kanal uchun xabar yozing:")
    await callback.answer()
...
async def handle_text(message: types.Message):
    ...
    if is_instagram_link(text):
        ...
        try:
            audio_url = await get_instagram_audio(text)
            if audio_url:
