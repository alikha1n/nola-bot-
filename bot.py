...
def init_db():
    ...
    # Migratsiya: bloklangan vaqti va qidiruv muvaffaqiyati
    try:
        c.execute("ALTER TABLE users ADD COLUMN blocked_at TEXT")
    except Exception:
        pass
    try:
        c.execute("ALTER TABLE searches ADD COLUMN success INTEGER DEFAULT 1")
    except Exception:
        pass
    conn.commit()
    conn.close()
...
def log_search(user_id, query, result, success=True):
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute("INSERT INTO searches (user_id, query, result, searched_at, success) VALUES (?, ?, ?, ?, ?)",
              (user_id, query, result, now, 1 if success else 0))
    conn.commit()
    conn.close()

def mark_blocked(user_id):
    """Foydalanuvchi botni bloklaganda chaqiriladi."""
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute("UPDATE users SET is_blocked=1, blocked_at=? WHERE user_id=?", (now, user_id))
    conn.commit()
    conn.close()
...
def get_blocked_stats():
    ...
    c.execute("SELECT COUNT(*) FROM users WHERE is_blocked=1 AND blocked_at LIKE ?", (f"{today}%",))
    today_blocked = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM users WHERE is_blocked=1 AND blocked_at LIKE ?", (f"{month}%",))
    month_blocked = c.fetchone()[0]
    conn.close()
    return total_blocked, today_blocked, month_blocked

def get_suspicious_stats():
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    c.execute("SELECT COUNT(*) FROM users")
    total = c.fetchone()[0] or 0
    c.execute("SELECT COUNT(*) FROM users WHERE user_id NOT IN (SELECT DISTINCT user_id FROM searches)")
    never_searched = c.fetchone()[0] or 0
    c.execute("SELECT COUNT(*) FROM users WHERE username IS NULL OR username=''")
    no_username = c.fetchone()[0] or 0
    c.execute("""SELECT COUNT(*) FROM users
                 WHERE is_blocked=1 AND blocked_at IS NOT NULL AND joined_at IS NOT NULL
                 AND (julianday(blocked_at) - julianday(joined_at)) <= 1""")
    fast_blocked = c.fetchone()[0] or 0
    conn.close()
    return total, never_searched, no_username, fast_blocked


def get_search_success_stats():
    conn = sqlite3.connect(DB)
    c = conn.cursor()
    today = datetime.now().strftime("%Y-%m-%d")
    month = datetime.now().strftime("%Y-%m")
    c.execute("SELECT COUNT(*) FROM searches WHERE success=1 AND searched_at LIKE ?", (f"{today}%",))
    today_ok = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM searches WHERE success=0 AND searched_at LIKE ?", (f"{today}%",))
    today_fail = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM searches WHERE success=1 AND searched_at LIKE ?", (f"{month}%",))
    month_ok = c.fetchone()[0]
    c.execute("SELECT COUNT(*) FROM searches WHERE success=0 AND searched_at LIKE ?", (f"{month}%",))
    month_fail = c.fetchone()[0]
    conn.close()
    return today_ok, today_fail, month_ok, month_fail
...
# Qo'shiq matni (lyrics) tugmasi va xabarlari — har tilda
_LYRICS_TEXTS = {
    "uz": {"btn_lyrics": "📝 Matn", "lyrics_loading": "📝 Matn yuklanmoqda...", "lyrics_none": "❌ Bu qo'shiq matni topilmadi"},
    "ru": {"btn_lyrics": "📝 Текст", "lyrics_loading": "📝 Загружаю текст...", "lyrics_none": "❌ Текст песни не найден"},
    "en": {"btn_lyrics": "📝 Lyrics", "lyrics_loading": "📝 Loading lyrics...", "lyrics_none": "❌ Lyrics not found"},
    "kk": {"btn_lyrics": "📝 Мәтін", "lyrics_loading": "📝 Мәтін жүктелуде...", "lyrics_none": "❌ Мәтін табылмады"},
    "ky": {"btn_lyrics": "📝 Текст", "lyrics_loading": "📝 Текст жүктөлүүдө...", "lyrics_none": "❌ Текст табылган жок"},
    "tg": {"btn_lyrics": "📝 Матн", "lyrics_loading": "📝 Матн боргирӣ мешавад...", "lyrics_none": "❌ Матн ёфт нашуд"},
    "tr": {"btn_lyrics": "📝 Sözler", "lyrics_loading": "📝 Sözler yükleniyor...", "lyrics_none": "❌ Şarkı sözü bulunamadı"},
    "tk": {"btn_lyrics": "📝 Sözler", "lyrics_loading": "📝 Sözler ýüklenýär...", "lyrics_none": "❌ Sözler tapylmady"},
    "az": {"btn_lyrics": "📝 Sözlər", "lyrics_loading": "📝 Sözlər yüklənir...", "lyrics_none": "❌ Sözlər tapılmadı"},
    "ar": {"btn_lyrics": "📝 الكلمات", "lyrics_loading": "📝 جاري تحميل الكلمات...", "lyrics_none": "❌ لم يتم العثور على الكلمات"},
}
for _lng, _d in _LYRICS_TEXTS.items():
    TEXTS.setdefault(_lng, {}).update(_d)
...
def admin_keyboard():
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🕵️ Shubhali foydalanuvchilar", callback_data="adm_suspicious")],
    ])
...
lyrics_cache = {}        # short_key -> (title, artist)  — "Matn" tugmasi uchun

def song_keyboard(uid, title, artist):
    """Yuborilgan qo'shiq tagidagi '📝 Matn' tugmasi (har tilda)."""
    key = hashlib.md5(f"{artist}|{title}".lower().encode()).hexdigest()[:16]
    lyrics_cache[key] = (title, artist)
    return InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=t(uid, "btn_lyrics"), callback_data=f"lyr|{key}")
    ]])

async def fetch_lyrics(title: str, artist: str) -> str | None:
    """Bepul API'lardan qo'shiq matnini oladi (lrclib → lyrics.ovh)."""
    # 1) lrclib.net (kalit shart emas)
    try:
        async with aiohttp.ClientSession() as s:
            params = {"track_name": title, "artist_name": artist}
            async with s.get("https://lrclib.net/api/get", params=params,
                             timeout=aiohttp.ClientTimeout(total=10)) as r:
                if r.status == 200:
                    d = await r.json()
                    ly = (d.get("plainLyrics") or "").strip()
                    if ly:
                        return ly
    except Exception:
        pass
...
def _find_dl(key: str) -> str | None:
    """/tmp/dl_<key>.* fayllaridan birinchi haqiqiysini topadi."""
    for f in sorted(glob.glob(f"/tmp/dl_{key}.*")):
        try:
...
async def ytdlp_download_url(url: str, key: str) -> str | None:
    """YouTube URL/ID dan audioni TO'G'RIDAN (qayta kodlamasdan) yuklaydi.
    mp3'ga o'tkazish ffmpeg'ni ishlatib sekinlashtirardi — endi audio o'z
    formatida (m4a) yuboriladi: SIFAT O'ZGARMAYDI, lekin ANCHA TEZ."""
    # Cache: shu qo'shiq oldin yuklangan bo'lsa — qaytadan yuklamaymiz
    existing = _find_dl(key)
    if existing:
        return existing
    ...
    for clients in client_variants:
        cmd = [
            ...
            # Qayta kodlash YO'Q — m4a audioni o'z holicha olamiz (tez + sifatli)
            "-f", "bestaudio/best",
    ...
    if cached:
        try:
            await bot.send_audio(uid, audio=cached, title=title[:64],
                                 ...
                                 reply_markup=song_keyboard(uid, title, artist))
            if extra_key:
...
async def cb_findsong(callback: types.CallbackQuery):
    ...
    try:
        ...
        if not result:
            ...
            log_search(uid, media_url, "", success=False)
            return
        ...
        if ok:
            ...
            await status.edit_text(t(uid, "dl_fail"))
        log_search(uid, media_url, s_title, success=ok)
    except Exception as e:
...
async def cb_lyrics(callback: types.CallbackQuery):
    uid = callback.from_user.id
    key = callback.data.split("|", 1)[1]
    pair = lyrics_cache.get(key)
    if not pair:
        await callback.answer(t(uid, "expired"), show_alert=True)
        return
    title, artist = pair
    await callback.answer(t(uid, "lyrics_loading"))
    ly = await fetch_lyrics(title, artist)
    if not ly:
        await callback.message.answer(t(uid, "lyrics_none"))
        return
    header = f"📝 <b>{title}</b>" + (f" — {artist}" if artist else "") + "\n\n"
    full = header + ly
    # Telegram xabar chegarasi ~4096 belgi — bo'laklarga bo'lib yuboramiz
    for i in range(0, len(full), 4000):
        try:
            await callback.message.answer(full[i:i+4000])
        except Exception:
            break
...
async def cb_stats(callback: types.CallbackQuery):
    ...
    total_blocked, today_blocked, month_blocked = get_blocked_stats()
    today_ok, today_fail, month_ok, month_fail = get_search_success_stats()
    text = (f"📊 <b>Statistika</b>\n\n"
            f"👥 Jami foydalanuvchilar: <b>{total}</b>\n"
            f"🆕 Bugun qo'shilganlar: <b>{today_new}</b>\n"
            f"🟢 Bugun faollar: <b>{today_active}</b>\n"
            f"🚫 Bloklaganlar (jami): <b>{total_blocked}</b>  (bugun: {today_blocked}, shu oy: {month_blocked})\n\n"
            f"🔍 Jami qidiruvlar: <b>{total_s}</b>\n"
            f"🔎 Bugungi qidiruvlar: <b>{today_s}</b>\n"
            f"✅ Bugun topilgan: <b>{today_ok}</b>   ❌ Topilmagan: <b>{today_fail}</b>\n"
            f"✅ Shu oy topilgan: <b>{month_ok}</b>   ❌ Topilmagan: <b>{month_fail}</b>\n\n"
            f"🕐 {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    await callback.message.edit_text(text, reply_markup=admin_keyboard())
    await callback.answer()

@dp.callback_query(F.data == "adm_suspicious")
async def cb_suspicious(callback: types.CallbackQuery):
    if not is_admin(callback.from_user.id):
        return
    total, never_searched, no_username, fast_blocked = get_suspicious_stats()
    pct = lambda x: f"{(x / total * 100):.1f}%" if total else "0%"
    text = (f"🕵️ <b>Shubhali faollik tahlili</b>\n\n"
            f"👥 Jami: <b>{total}</b>\n"
            f"🔇 Hech qidiruv qilmagan: <b>{never_searched}</b> ({pct(never_searched)})\n"
            f"🙈 Username'siz: <b>{no_username}</b> ({pct(no_username)})\n"
            f"⚡ 24 soat ichida bloklagan: <b>{fast_blocked}</b>\n\n"
            f"<i>Eslatma: bu yakka o'zi \"bot\" ekanini isbotlamaydi, lekin yuqori foiz — "
            f"sun'iy/nakrutka auditoriya belgisi bo'lishi mumkin.</i>")
    await callback.message.edit_text(text, reply_markup=admin_keyboard())
    await callback.answer()
...
async def confirm_bc(callback: types.CallbackQuery, state: FSMContext):
    ...
    if target == "channel":
        ...
        users = get_all_users()
        sem = asyncio.Semaphore(25)  # bir vaqtda 25 ta parallel yuborish
        ...
        for i in range(0, len(users), BATCH):
            await asyncio.gather(*[_send_one(uid) for uid in users[i:i + BATCH]])
            try:
                done = counters["sent"] + counters["failed"]
                await status_msg.edit_text(f"📤 Yuborilmoqda... {done}/{len(users)}")
            except Exception:
                pass
...
async def handle_audio(message: types.Message):
    ...
    try:
        ...
        if not result:
            ...
            log_search(uid, "audio", "", success=False)
            return
        ...
        if not songs:
            await status.edit_text(t(uid, "found_shazam", title=s_title, artist=s_artist) +
                                   "\n\n" + t(uid, "no_result"))
            log_search(uid, "audio", s_title, success=False)
            return
