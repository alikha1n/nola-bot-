...
# ==================== NAVBAT TIZIMI ====================
# Bir vaqtda max 8 ta yuklash ishlaydi (4CPU/8GB serverga mos).
# User hech qachon "navbat" so'zini ko'rmaydi — doim "Yuklanmoqda..." turadi,
# faqat orqa fonda kim birinchi so'rasa birinchi tayyor bo'ladi (FIFO).
MAX_PARALLEL_DOWNLOADS = 8
download_semaphore = asyncio.Semaphore(MAX_PARALLEL_DOWNLOADS)

class DownloadSlot:
    """Bo'sh joy chiqishini kutadi, lekin foydalanuvchiga buni bildirmaydi."""
    def __init__(self):
        self.got_slot = False
...
async def ytdlp_download(title: str, artist: str) -> str | None:
    ...
    Tezlik uchun: re-encode qilmasdan tayyor audio formatni olamiz
    (YouTube'dagi m4a/webm audio odatda 128-160kbps, sifat yetarli)."""
    ...
    cmd = [
        ...
        "-x", "--audio-format", "mp3",
        # Qayta kodlamasdan, faqat kontainer/format almashtirish (tez):
...
async def get_instagram_audio(ig_url: str) -> str | None:
    """Media (IG/TikTok/YT) dan audioni yuklaydi. Ishlamasa None qaytaradi."""
    h = hashlib.md5(ig_url.encode()).hexdigest()[:16]
    out = f"/tmp/ig_{h}.mp3"
    if os.path.exists(out):
        return out

    # Turli usullar bilan urinib ko'ramiz (bu — faqat Shazam uchun, sifat shart emas, tezlik muhim)
    attempts = [
        # 1) Oddiy yuklash — past sifat, tez
        [YTDLP, "-x", "--audio-format", "mp3", "--audio-quality", "9",
         "-f", "worstaudio/bestaudio", "--no-playlist", "--no-warnings",
         "--no-check-certificate", "--max-filesize", "20M",
         "-o", f"/tmp/ig_{h}.%(ext)s", ig_url],
        # 2) User-agent bilan (bot detection chetlab o'tish)
        [YTDLP, "-x", "--audio-format", "mp3", "--audio-quality", "9",
         "-f", "worstaudio/bestaudio", "--no-playlist", "--no-warnings",
         "--no-check-certificate", "--max-filesize", "20M",
...
async def handle_text(message: types.Message):
    ...
    if media_url:
        ...
        try:
            ...
            # 1) AudD ga URL ni to'g'ridan-to'g'ri yuboramiz (fayl yuklamasdan!)
            result = await recognize_from_url(media_url)

            # 2) Ishlamasa — audio yuklab AudD ga yuboramiz
            if not result:
                async with DownloadSlot():
            ...
            if cached:
                try:
                    await bot.send_audio(uid, audio=cached,
                        title=s_title[:64], performer=s_artist, caption=cap)
                    save_cached_audio(link_key, cached)
                    await status.delete()
                    return
            ...
            # YouTube dan to'liq 320K yuklaymiz
            async with DownloadSlot():
                full = await ytdlp_download(s_title, s_artist)
            if full:
                msg = await bot.send_audio(uid, audio=FSInputFile(full),
