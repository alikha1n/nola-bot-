        ...
        with open(file_path, "rb") as f:
            ...
            async with http().post("https://api.audd.io/", data=data,
                ...
                if result.get("status") == "success" and result.get("result"):
                    r = result["result"]
                    # Albom rasmini (artwork) Apple Music yoki Spotify'dan olamiz
                    cover = None
...
async def handle_audio(message: types.Message):
    ...
    try:
        ...
        # Topilgan nom bo'yicha YOUTUBE'da qidiramiz. "official audio" qo'shamiz —
        # bu asl/original versiyani topadi (remix, cover, mashup emas).
        # Avval "official audio" bilan, topilmasa oddiy nom bilan qidiramiz.
        base_query = f"{s_artist} {s_title}".strip()
        yt_query = f"{base_query} official audio"
        songs = await youtube_search(yt_query, limit=12)
        if not songs:
            # "official audio" topilmasa — oddiy nom bilan qidiramiz
            yt_query = base_query
            songs = await youtube_search(yt_query, limit=12)
        await stop_anim(anim)
        ...
        # === TINGLA USLUBI: albom rasmi + ijrochi/nom + raqamli ro'yxat ===
        cover = result.get("cover")
        header = (f"🎤 <b>Ijrochi:</b> {s_artist}\n"
                  f"🎵 <b>Qo'shiq nomi:</b> {s_title}\n\n")
        body = header + results_text(songs, 0, uid)
