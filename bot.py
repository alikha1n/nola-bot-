...
async def handle_text(message: types.Message):
    ...
    if media_url:
        ...
        # === YouTube link → audioni TO'G'RIDAN-TO'G'RI yuklaydi ===
        # (tanish/qidirish shart emas — link allaqachon aniq qo'shiqni ko'rsatadi)
        status = await message.answer("⏳")
        anim = asyncio.create_task(animate_loading(status, t(uid, "anim_downloading")))
        link_key = "lnk_" + hashlib.md5(media_url.encode()).hexdigest()

        # Cache: shu link oldin yuklanganmi?
        cached_song = get_cached_audio(link_key)
        if cached_song:
            try:
                await bot.send_audio(uid, audio=cached_song,
        ...
        try:
            yt_key = hashlib.md5(media_url.encode()).hexdigest()[:16]
            # Sarlavha + ijrochi (metadata) va audioni parallel olamiz — tezroq
            info_task = asyncio.create_task(ytdlp_get_info(media_url))
            path = await ytdlp_download_url(media_url, yt_key)
            ...
            try:
                msg = await bot.send_audio(uid, audio=FSInputFile(path),
                                           title=title[:64], performer=artist or None,
                                           caption=cap)
            ...
            await status.delete()
            log_search(uid, media_url, title)
        except Exception as e:
            logger.error(f"YouTube link error: {e}")
            await stop_anim(anim)
            await status.edit_text(t(uid, "error"))
        return
