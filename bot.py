...
async def ytdlp_download(title: str, artist: str) -> str | None:
    ...
    if os.path.exists(out):
        return out
    cmd = [
        YTDLP, "-x", "--audio-format", "mp3", "--audio-quality", "128K",
...
def detect_media_link(text: str) -> str | None:
    """YouTube/TikTok/Instagram havolasini aniqlaydi. URL ni qaytaradi."""
    patterns = [
...
async def get_instagram_audio(ig_url: str) -> str | None:
    """Instagram dan audioni yt-dlp bilan faylga yuklaydi. Fayl yo'lini qaytaradi."""
...
async def handle_text(message: types.Message):
    ...
    if media_url:
        status = await message.answer("⏳ Yuklanmoqda... / Загрузка...")
        try:
            audio_path = await get_instagram_audio(media_url)  # universal yt-dlp yuklovchi
            if audio_path:
                # Avval Shazam bilan aniqlaymiz
                result = await recognize_from_file(audio_path)
                if not result:
                    # Topilmasa — hech bo'lmasa videoning o'z audiosini yuboramiz
                    try:
                        await bot.send_audio(
                            uid, audio=FSInputFile(audio_path),
                            caption="🎵 Audio tayyor!\n\n🤖 @nolabot"
                        )
                    except Exception as e:
                        logger.error(f"send media audio: {e}")
                try:
                ...
                if result:
                    ...
                    # 2) YouTube dan to'liq yuklash
                    if not sent:
                        full = await ytdlp_download(s_title, s_artist)
                        if full:
                            msg = await bot.send_audio(
                                uid, audio=FSInputFile(full),
                    ...
                    # 3) Bo'lmasa Deezer ro'yxati (zaxira)
                    if not sent:
                        songs = await search_songs(f"{s_artist} {s_title}", limit=20)
                        if not songs:
                            songs = await search_songs(s_title, limit=20)
                        if songs:
                            search_cache[uid] = songs
                            total_pages = (len(songs) + 9) // 10
                            info = t(uid, "page_info", cur=1, total=total_pages, count=len(songs))
